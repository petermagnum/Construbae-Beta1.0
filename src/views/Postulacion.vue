    ...
    <v-layout wrap>
      <v-flex class="mt-5" xs12>
        ...
        <h2 class="ml-5 mr-5 mb-5 mt-5" align="justify">
          Para postular debe cumplir con ciertos requisitos, los cuales tendrá
          que detallar en el siguiente formulario.
        </h2>
      </v-flex>
    </v-layout>
        ...
        <v-row class="justify-center pt-8 pb-8">
          <v-card-actions>
            <v-btn
              color="blue"
              type="submit"
              @click="enviarFormulario(formulario)"
            >
              Enviar Formulario
            </v-btn>
          </v-card-actions>
    ...
    async enviarFormulario(formulario) {
      try {
        const res = await fetch(
          `https://webproyect-27e28-default-rtdb.firebaseio.com/formulario/${formulario.rut}.json`,
          {
            method: "PUT",
            header: {
